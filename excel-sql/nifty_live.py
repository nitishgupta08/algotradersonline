...
def export_to_sql(my_conn):
    ...
    excel_df = nitfty_sheet.range("A6:V35").options(pd.DataFrame).value
    ...
    excel_df.to_sql(con=my_conn,name="optionchain_nifty",if_exists="replace",index=False)
    print(f"NIFTY live updated to database at {dt.datetime.now().strftime('%H:%M:%S')}")
