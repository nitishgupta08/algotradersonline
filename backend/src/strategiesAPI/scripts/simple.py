...
import django
django.setup()
from ..models import Papertrade, TradedStocks, FiveMIN
...
def main():
    for name in instrument_list:
        vo = test(name)
        q = FiveMIN.objects.all().get(name=name)
    ...
    while ((datetime.time(9, 20, 5) <= datetime.datetime.now().time()
        ...
        for name in instrument_list:
            q = FiveMIN.objects.get(name=name)
...
#                     and (range_oc2 > range_hl2 * 0.80) and (close - open < 0.03 * open) and (close > atp)
            if ((name not in traded_stocks) and (vol > vo)):
                traded_stocks.append(name)
                print(f"Real Entry {name} {vol} {vo}")
            ...
            if ((name not in traded_stocks) and (vol > vo)):
                print(f"Real Exit {name} {name} {vol} {vo}")
                traded_stocks.append(name)
        ...
        for name in instrument_list:
            ...
            if ((not TradedStocks.objects.filter(username=algotrade_username).filter(stock_name=name).exists()) and (vol > vo) and (open < close)
                ...
                Papertrade.objects.create(start_time=datetime.datetime.now().time().strftime("%H:%M"),username=algotrade_username, signal='BUY', name=name, quantity=quantity_b,
                                          buy_price=high+((high-low)/4), sell_price=0, stop_loss=stop_loss, target=square_off,historical_volume=vo,current_volume=vol)
            ...
            if ((not TradedStocks.objects.filter(username=algotrade_username).filter(stock_name=name).exists()) and (vol > vo) and (open > close)
                ...
                Papertrade.objects.create(start_time=datetime.datetime.now().time().strftime("%H:%M"),username=algotrade_username, signal='SELL', name=name, quantity=quantity_s,
                                          buy_price=0, sell_price=low-((high-low)/4), stop_loss=stop_loss, target=square_off,historical_volume=vo,current_volume=vol)
